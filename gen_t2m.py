...
def load_trans_model(model_opt, opt, which_model):
    ...
    #                   map_location='cpu')
    ckpt = torch.load('/mnt/cap/karong/t2m/momask-codes/checkpoints/t2m/mtrans_name_ca_ar/model/net_best_fid.tar',
...
def load_len_estimator(opt):
    ...
    ckpt = torch.load(pjoin(opt.checkpoints_dir, opt.dataset_name, 'length_estimator', 'length_estimator', 'model', 'finest.tar'),
                      map_location=opt.device)
...
if __name__ == '__main__':
    ...
    if est_length:
        ...
        text = clip.tokenize(prompt_list, truncate=True).to('cuda:0')
    ...
    for r in range(opt.repeat_times):
        ...
        with torch.no_grad():
            ...
            pred_pose_eval = torch.zeros((13, 196, 263)).cuda()
            pred_len = torch.ones(13).long()
            # print(mids)
            # print(mids.shape)
            for k in range(mids.size(0)):
                pred_pose = vq_model.forward_decoder(mids[k][:(token_lens)[k]])
                cur_len = pred_pose.shape[1]
                pred_len[k] = min(cur_len, 196)
                pred_pose_eval[k:k+1, :cur_len] = pred_pose[:, :196]
            # mids = res_model.generate(mids, captions, token_lens, temperature=1, cond_scale=5)
            # pred_motions = vq_model.forward_decoder(mids)

            pred_motions = pred_pose_eval.detach().cpu().numpy()
