...
def load_trans_model(model_opt, which_model):
    ...
    t2m_transformer = MaskTransformer(vq_model=vq_model,
    ...
    #                   map_location=opt.device)
    # ckpt = torch.load('/mnt/cap/karong/t2m/momask-codes/checkpoints/t2m/mtrans_name_lat512/model/net_best_fid.tar',
    #                   map_location=opt.device)
    ckpt = torch.load('../mld_st/src/net_best_fid.tar',
                      map_location=opt.device)
...
def load_res_model():
    ...
    res_transformer = trans.Text2Motion_Transformer(num_vq=vq_opt.nb_code, 
                                vq_model=None,
                                embed_dim=1024, 
                                clip_dim=768, 
                                block_size=51, 
    ...
    ckpt = torch.load('/mnt/workspace/cap/karong/t2m/output/GPTBestFormer_t2m/net_best_fid.pth',
                      map_location=opt.device)
    res_transformer.load_state_dict(ckpt['trans'], strict=False)
            ...
            with torch.no_grad():
                best_fid, best_div, Rprecision, best_matching, best_mm = \
                    eval_t2m.evaluation_mask_transformer_test(eval_val_loader, vq_model, t2m_transformer,
                                                         ...
                                                         temperature=opt.temperature, topkr=opt.topkr, gsample=True,
