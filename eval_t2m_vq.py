...
def load_vq_model(vq_opt, which_epoch):
    ...
    #                         map_location='cpu')
    ckpt = torch.load('./pretrained/net_best_fid.tar', map_location='cpu')
...
if __name__ == "__main__":
    ...
    eval_val_loader, eval_dataset = get_dataset_motion_loader(dataset_opt_path, 32, 'test', device=args.device)
    ...
    for file in os.listdir(model_dir):
        ...
        for i in range(repeat_time):
            best_fid, best_div, Rprecision, best_matching, l1_dist = \
                eval_t2m.evaluation_vqvae_plus_mpjpe(eval_dataset, eval_val_loader, net, i, eval_wrapper=eval_wrapper, num_joint=args.nb_joints)
            fid.append(best_fid)
            div.append(best_div)
