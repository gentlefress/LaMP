...
def lengths_to_mask(lengths, max_len):
    # max_len = max(lengths)
    # mask = torch.arange(max_len, device='cuda:0').view(-1, max_len) < lengths
    mask = torch.arange(max_len, device='cuda:7').expand(len(lengths), max_len) < lengths.unsqueeze(1)
    return mask #(b, len)
...
def get_subsequent_mask(seq):
    ...
    return subsequent_mask.to('cuda:0')
