...
class RVQTokenizerTrainer:
    ...
    # def forward(self, batch_data, temperature):
    def forward(self, batch_data):
        motions, caption = batch_data
        motions = motions.cuda().float()
        # pred_motion, loss_commit, code_idx = self.vq_model(motions, mode='train', temperature=temperature)
        pred_motion, loss_commit, perplexity = self.vq_model(motions, caption)
        ...
        # return loss, loss_rec, loss_explicit, loss_commit, code_idx
    ...
    def temperature_schedular(self, batch_idx):
        ...
        # linear_decay = 1 - batch_idx / total_steps
        # new_lr = (1+math.cos(batch_idx/circle_steps*math.pi))*linear_decay*initial_lr
        ...
        if x<0.48:
            ...
            linear_decay = 0.04 - 0.04*(x-0.48)/(1-0.48)
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval=None):
        ...
        # while epoch < self.opt.max_epoch:
            ...
            for i, batch_data in enumerate(train_loader):
                ...
                # temperature = 1.0
                # loss, loss_rec, loss_vel, loss_commit, code_idx = self.forward(batch_data, temperature)
                # loss_commit = torch.tensor(loss_commit)
            ...
            with torch.no_grad():
                for i, batch_data in enumerate(val_loader):
                    loss, loss_rec, loss_vel, loss_commit, perplexity = self.forward(batch_data)
                    # loss, loss_rec, loss_vel, loss_commit, code_idx = self.forward(batch_data, temperature=1.0)
                    # loss_commit = torch.tensor(loss_commit)
